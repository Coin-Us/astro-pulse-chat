-- ============================================
-- ALCHEMY WHALE TRACKING DATABASE SETUP
...
create table if not exists public.wallet_transfers (
  ...
  constraint valid_direction check (transfer_direction in ('inflow', 'outflow', 'exchange_to_wallet', 'wallet_to_exchange', 'wallet_to_wallet', 'exchange_to_exchange'))
);

-- Indexes for performance
create index if not exists wallet_transfers_timestamp_idx on public.wallet_transfers(timestamp desc);
create index if not exists wallet_transfers_from_address_idx on public.wallet_transfers(from_address);
create index if not exists wallet_transfers_to_address_idx on public.wallet_transfers(to_address);
create index if not exists wallet_transfers_asset_symbol_idx on public.wallet_transfers(asset_symbol);
create index if not exists wallet_transfers_significant_idx on public.wallet_transfers(is_significant) where is_significant = true;
create index if not exists wallet_transfers_chain_idx on public.wallet_transfers(chain_id, network);

-- Enable Row Level Security
alter table public.wallet_transfers enable row level security;

-- Drop existing policies if they exist
drop policy if exists "Transfer data is public" on public.wallet_transfers;
drop policy if exists "Service can insert transfers" on public.wallet_transfers;
...
-- Indexes
create index if not exists whale_wallets_address_idx on public.whale_wallets(address);
create index if not exists whale_wallets_active_idx on public.whale_wallets(is_active) where is_active = true;

-- Enable RLS
alter table public.whale_wallets enable row level security;

-- Drop existing policies if they exist
drop policy if exists "Whale wallet data is public" on public.whale_wallets;
...
-- Indexes
create index if not exists transfer_signals_asset_idx on public.transfer_signals(asset_symbol, created_at desc);
create index if not exists transfer_signals_expires_idx on public.transfer_signals(expires_at);

-- Enable RLS
alter table public.transfer_signals enable row level security;

-- Drop existing policies if they exist
drop policy if exists "Signals are public" on public.transfer_signals;
...
drop trigger if exists update_whale_metrics_trigger on wallet_transfers;
drop function if exists update_whale_activity_metrics();
